...
def _stringify_answer(question: dict[str, Any]) -> str:
    """Convert any of Cliniko's answer shapes into a flat text string."""
    # Direct fields
        ...
        if isinstance(val, str):
            ...
            for item in val:
                if isinstance(item, str) and item.strip():
                    ...
                    # Could be {"label": "Knee", "checked": true} or just
                    # {"value": "Knee"}.
    ...
    # "options" with embedded checked state — common variant
    ...
    if isinstance(options, list):
        ...
        for opt in options:
            if not isinstance(opt, dict):
                continue
            if opt.get("checked") or opt.get("selected"):
...
def extract_injuries_from_note(note: dict[str, Any],
    ...
    # Pass A — checkbox answers go straight to classifier (no need to
    # be defensive about false positives because the text comes from
    # template options that we control via settings.yml keywords).
    if checkbox_text:
        ...
        # unmappable like "" or unexpected text.

    # Pass B — keyword scan of HoPC paragraph
    if hopc_text:
        return classify_injury(hopc_text, patterns_by_cat)
