...
from .config import load_settings, load_rap_exempt
...
# -------------------------------------------------------------------
def _get_patient(client: ClinikoClient, patient_id: int) -> dict[str, Any]:
    return client.get(f"patients/{patient_id}")


def _get_attachments(client: ClinikoClient, patient_id: int) -> list[dict[str, Any]]:
    try:
...
def _get_letters(client: ClinikoClient, patient_id: int) -> list[dict[str, Any]]:
    try:
...
def _get_patient_treatment_notes(client: ClinikoClient, patient_id: int) -> list[dict[str, Any]]:
    try:
...
def _get_patient_appointments(client: ClinikoClient, patient_id: int) -> list[dict[str, Any]]:
    try:
...
def _get_recalls(client: ClinikoClient, patient_id: int) -> list[dict[str, Any]]:
    try:
    ...
    for i, row in enumerate(pool.itertuples(index=False), start=1):
        try:
            r = audit_patient(
                client,
                int(row.patient_id),
                int(row.practitioner_id),
                int(row.business_id) if pd.notna(row.business_id) else None,
                row.cohort,
                practitioners,
            )
            results.append(r)
            ...
            results.append(PatientAudit(
                ...
                business_id=int(row.business_id) if pd.notna(row.business_id) else None,
                cohort=row.cohort,
