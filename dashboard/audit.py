...
def _get_letters(client: ClinikoClient, patient_id: str) -> list[dict[str, Any]]:
    # v20 — the nested path /patients/{id}/letters 404s on this Cliniko shard
    # (confirmed against Matt's au1 shard 2026-04-24 — 278/278 error rate).
    # Cliniko only exposes letters as a top-level collection; filter via
    # q[]=patient_id:=<id>. Same pattern switch applied to appointments and
    # recalls below.
    try:
