...
def overview_tab(filters: dict):
    ...
    with st.expander("Raw scored table"):
        ...
        if notes_source:
            ...
            st.caption(
                f"{icon} Notes source: **{notes_source}**"
                + (f" (fetched {notes_count} notes)" if notes_count is not None else "")
                + (". If source is 'fallback', the /treatment_notes endpoint "
                   "returned no data — numbers will be pessimistic (same "
                   "behaviour as v18 and earlier)." if "fallback" in notes_source else "")
            )
