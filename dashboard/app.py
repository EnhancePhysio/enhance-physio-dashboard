...
def _cached_clinical_review(_schema_version: str = ""):
    """30-min cache around the clinical-review computation. The
    parameter is a placeholder for future cache-bust keys."""
    from dashboard.clinical_review import compute_clinical_review
    return compute_clinical_review(get_client())
...
def _cached_commission(year: int, month: int, _schema_version: str = ""):
    """30-min cache around the heavy invoice fetch. Manual adjustments
    are NOT cached — they're applied after the cache layer."""
...
def commission_tab(filters: dict) -> None:
    ...
    with st.expander("🔍 Diagnostic: revenue fetch + name match", expanded=False):
        ...
        if st.button("Run invoice-items diagnostic (extra ~30s API call)",
            ...
            st.caption(
                f"Appointments fetched in window: **{diag['appts_fetched']}** · "
            ...
            if diag.get("first_raw_shape"):
                ...
                st.caption(
                    ...
                    "patient data. If `appointment` isn't in this list, "
                    "Cliniko links the appt under a different field name."
                )
