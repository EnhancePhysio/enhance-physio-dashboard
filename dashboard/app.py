            ...
            try:
                ...
                for p in client.paginate(
                    ...
                    if start_utc <= c_dt < end_utc:
                        created_hits += 1
                st.write(
                    f"Patients created inside the range "
                    f"(DVA/NDIS/TAC/WC new-patient signal): **{created_hits}** "
                    f"(scanned {scanned} recently-updated patient records)"
                )
            except Exception as e:
