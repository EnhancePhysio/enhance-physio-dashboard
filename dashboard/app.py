        ...
        try:
            ...
            if not types.empty:
                ...
                if not initials.empty:
                    ...
                    st.warning(
                        "No appointment types matched the Initial pattern. "
                        "new_patients will be 0 for everyone. Paste your actual "
                        "appointment-type names here — the regex can be widened."
                    )
                    st.write("All appointment type names (for reference):")
                    st.dataframe(types[["id", "name"]], hide_index=True)
        except Exception as e:
