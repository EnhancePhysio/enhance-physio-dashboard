...
def _normalise_image_to_jpeg(image_bytes: bytes) -> tuple[bytes, str]:
    ...
    # Register HEIC/HEIF support if the plugin is available. Fails silently
    # on environments that don't need it — non-HEIC files open regardless.
    try:
    ...
    try:
        ...
        raise RuntimeError(
            "Couldn't read the uploaded file as an image. "
