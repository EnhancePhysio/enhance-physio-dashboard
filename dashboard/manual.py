...
# -------------------------------------------------------------------
# v26.7.1 — a context header is prepended to this at call-time so the vision
# model knows the current year. Without that, handwritten sheets labelled
# "23/3 - 27/3" (no year anywhere) would produce week_starting dates from
# the model's training data (often 2024), which then fell outside any
# "Last month" filter applied in 2026 and silently dropped every row.
...
The sheet header usually shows a week range like "23/3 - 27/3" with NO year.
You will be told today's date in the user message preamble. When the sheet
shows only day/month, use the CURRENT YEAR from that preamble. If the
day/month suggests a date in the future (e.g. today is 2026-04-25 and the
sheet says 15/6), then the sheet is from the previous year — use
CURRENT_YEAR - 1. Dates are always DAY/MONTH (Australian format), never
MONTH/DAY.
  • Today 2026-04-25, sheet "23/3 - 27/3" → week_starting 2026-03-23
  • Today 2026-04-25, sheet "15/6 - 19/6" → week_starting 2025-06-15
  • Today 2026-04-25, sheet "6/4 - 10/4" → week_starting 2026-04-06
NEVER default to a year from your training data. The preamble tells you
the real current year — use it.
...
def _coerce_week_starting_year(week_starting: str,
                                today: date | None = None) -> str:
    """Snap a vision-extracted week_starting into a plausible year.

    The vision model sometimes returns the right month/day but a wrong year
    (usually 2024, inherited from training data) when the sheet itself has
    no year printed. This post-pass forces the date into the window
    [today - 13 months, today]:

      * If the parsed date is already inside that window → unchanged.
      * If it's MORE than 13 months old → bump the year forward until it
        lands inside the window. Prefers the MOST RECENT matching year
        (so a "23/3" sheet uploaded in April 2026 becomes 2026-03-23,
        not 2025-03-23 or 2024-03-23).
      * If the parsed date is in the FUTURE relative to today → subtract
        a year (handles back-fill of prior-year end-of-year sheets, e.g.
        sheet "15/12" uploaded in Jan → CURRENT_YEAR-1).
      * If parsing fails entirely → return as-is (downstream filter will
        drop the row, and the data editor lets Matt fix by hand).

    13 months (not 12) so a Dec sheet uploaded in early Feb still parses
    as the prior year rather than getting bumped forward to the future.

    v26.7.1 — the companion fix to the vision prompt's date-handling rules.
    Belt-and-braces: even if the AI ignores the preamble, this clamp
    brings the date back into sensible territory.
    """
    if not week_starting or not isinstance(week_starting, str):
        return week_starting
    today = today or date.today()
    try:
        parsed = pd.to_datetime(week_starting, errors="raise").date()
    except (ValueError, TypeError):
        return week_starting  # can't parse — leave alone, let filter drop it

    thirteen_months_ago = today - timedelta(days=395)  # ~13 months
    # Already inside the window → nothing to do.
    if thirteen_months_ago <= parsed <= today:
        return parsed.isoformat()
    # Future → roll back one year (handles Dec sheets uploaded in Jan).
    if parsed > today:
        try:
            return parsed.replace(year=parsed.year - 1).isoformat()
        except ValueError:  # Feb 29 corner
            return (parsed.replace(day=28, year=parsed.year - 1)).isoformat()
    # Too old → bump year forward to the most recent match that's still ≤ today.
    target_year = today.year
    while target_year > parsed.year:
        try:
            candidate = parsed.replace(year=target_year)
        except ValueError:
            candidate = parsed.replace(day=28, year=target_year)
        if candidate <= today:
...
def _repair_week_starting_in_response(response: dict[str, Any],
                                       today: date | None = None) -> dict[str, Any]:
    """Apply ``_coerce_week_starting_year`` in-place to the parsed response."""
    if not isinstance(response, dict):
        return response
    raw = response.get("week_starting", "")
    if raw:
        fixed = _coerce_week_starting_year(str(raw), today=today)
        if fixed != raw:
            response["week_starting"] = fixed
            # Record what was changed so the UI banner can surface it —
            # useful for the reviewer to double-check edge cases.
            response.setdefault("_meta", {})["week_starting_was"] = raw
    return response


def extract_punctuality_from_image(image_bytes: bytes, media_type: str = "image/jpeg",
                                    filename_hint: str | None = None) -> dict[str, Any]:
    ...
    v26.7.1 additions:
      * Prepends a preamble to the vision prompt with today's date and
        (if given) the upload filename. Fixes the year-guessing bug
        where sheets labelled only "23/3 - 27/3" were extracted with
        week_starting=2024-03-23 because the AI inferred from training
        data rather than the current year.
      * Post-processes the response through ``_coerce_week_starting_year``
        so wrong-year extractions get snapped back to a plausible window
    ...
    # v26.7.1 — Build a per-call preamble with today's date so the model
    # has a concrete anchor for year inference. The filename_hint (if given)
    # often contains the week number — we pass it through verbatim so the
    # model can cross-reference against the header it sees on the sheet.
    today = date.today()
    preamble_bits = [
    ...
    if filename_hint:
        preamble_bits.append(f"UPLOAD_FILENAME: {filename_hint}")
    preamble = (
        "CONTEXT PREAMBLE (read before processing):\n"
        + "\n".join(preamble_bits)
        + "\n\nWhen the handwritten header shows only day/month (no year), use "
          "CURRENT_YEAR unless the resulting date would be in the future — "
          "in which case use CURRENT_YEAR - 1. Never default to a year from "
          "your training data.\n\n---\n\n"
    )
    full_prompt = preamble + PUNCTUALITY_VISION_PROMPT
    ...
    try:
        return _repair_week_starting_in_response(json.loads(text), today=today)
    ...
    if extracted is not None:
        try:
            return _repair_week_starting_in_response(json.loads(extracted), today=today)
        except json.JSONDecodeError:
            pass
    raise RuntimeError(
