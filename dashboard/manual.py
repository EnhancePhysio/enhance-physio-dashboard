...
def _normalise_image_to_jpeg(image_bytes: bytes) -> tuple[bytes, str]:
    ...
    # Catch PDFs early with a clear message — PIL can't open them, and
    # rendering PDF pages would need pdf2image + poppler which isn't
    # installed on Streamlit Cloud by default.
    if image_bytes[:5] == b"%PDF-":
        raise RuntimeError(
            "PDF uploads aren't supported by the vision path yet — PIL "
            "can't render PDF pages. Open the PDF, screenshot the page "
            "(or 'Save as JPG'), and upload the image instead."
        )
