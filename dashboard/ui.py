...
def matrix_scatter(scored: pd.DataFrame, title: str = "Clinical × Non-clinical") -> go.Figure:
    ...
    for zone, group in df.groupby("zone"):
        fig.add_trace(go.Scatter(
            x=group["clinical_score"],
            y=group["nonclinical_score"],
            mode="markers+text",
            text=group["label"],
            ...
            customdata=group[[
                ...
                "nps_band", "audit_pct_band", "notes_completion_band",
    ...
    fig.update_layout(
        title=title,
        xaxis=dict(title="Clinical score (band avg, 0–10)", range=[0, 10], dtick=1),
        yaxis=dict(title="Non-clinical score (band avg, 0–10)", range=[0, 10], dtick=1),
        height=600,
        showlegend=True,
...
def practitioner_detail(scored: pd.DataFrame, practitioner_label: str) -> None:
    ...
    with col2:
        ...
        for m in NONCLINICAL_METRICS:
            ...
            if m == "nps":
                # nps stored as 0..1 (rubric-normalised); display raw if present
                raw = r.get("nps_raw", value * 200 - 100)
                val_str = f"{raw:.0f} (rubric-scaled {value * 100:.0f}%)"
            else:
                val_str = f"{value * 100:.1f}%"
            st.write(f"{label}: **{val_str}** → band **{band}/10**")
        st.write(f"**Non-clinical avg: {r['nonclinical_axis']:.1f}/10**")
