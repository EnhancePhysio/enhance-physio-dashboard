...
def band_score(metric_key: str, value: float | None) -> int:
    """Return the 1-10 band score for a raw metric value. NaN → 0 (missing)."""
...
def score_table(wide: pd.DataFrame) -> pd.DataFrame:
    ...
    for metric, src_col in METRIC_VALUE_COLS.items():
        if src_col not in out.columns:
            out[src_col] = 0.0
        out[f"{metric}_band"] = out[src_col].apply(lambda v: band_score(metric, v))
