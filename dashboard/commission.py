    ...
    try:
        for it in client.paginate("invoice_items", params=params):
            ...
            if first_raw is None:
                # Only retain key NAMES, not values, in case the value
                # contains PHI we shouldn't surface in the UI. We strip
                # nested dicts to a list-of-keys preview.
                first_raw = _safe_shape(it)
            aid = _appt_id_from_invoice_item(it)
            if not aid:
                counters["no_appt_link"] += 1
                continue
