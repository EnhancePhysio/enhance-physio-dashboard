...
def fetch_invoice_items_diagnostic(client: ClinikoClient,
    ...
    # v26.10.4 — pull invoices for the month so we can map invoice_id
    # → appt_id (Cliniko's invoice_item doesn't expose appointment directly).
    invoices = fetch_invoices_for_month(client, year, month)
    invoice_to_appt: dict[str, str] = {}
    sample_invoice: dict[str, Any] | None = None
    for _, r in invoices.iterrows():
        iid = str(r.get("invoice_id") or "")
        aid = str(r.get("appointment_id") or "")
        if iid and aid:
            invoice_to_appt[iid] = aid
    ...
    try:
        for it in client.paginate("invoice_items", params=params):
            ...
            if not inv_id:
                ...
                continue
            aid = invoice_to_appt.get(inv_id)
            if not aid:
...
def fetch_invoices_for_month(client: ClinikoClient,
    ...
    """Pull /invoices created in the month with their appointment link
    intact. Excludes archived/deleted/cancelled invoices.

    Returned DataFrame columns:
      invoice_id, appointment_id, status, total
    """
    dr = _month_range(year, month)
    params = {"q[]": [f"created_at:>={dr.start_iso_utc}",
                       f"created_at:<{dr.end_iso_utc}"]}
...
def fetch_invoice_items_for_month(client: ClinikoClient,
    ...
    # v26.10.4 — pull /invoices for the month and build invoice_id → appt_id.
    invoices = fetch_invoices_for_month(client, year, month)
    invoice_to_appt: dict[str, str] = {}
    for _, r in invoices.iterrows():
        iid = str(r.get("invoice_id") or "")
    ...
    try:
        for it in client.paginate("invoice_items", params=params):
            ...
            aid = invoice_to_appt.get(inv_id)
            if not aid:
                continue  # invoice with no appointment (standalone product / desk fee)
            meta = appt_map.get(aid)
            if meta is None:
