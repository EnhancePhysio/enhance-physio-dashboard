...
def _compute_white_windows(client: ClinikoClient, appts: pd.DataFrame,
    ...
    cols = ["practitioner_id", "business_id", "local_date",
            ...
            "qual_unavail_min", "nonqual_unavail_min",
            "break_min", "leave_min"]
    if appts.empty:
    ...
    # v26.7 — leave patterns (regex) also subtract from days_worked AND from
    # the utilisation denominator. Compiled once; checked before qualifying
    # so a block labelled "AL billable" is treated as leave, not admin.
    _leave_src = settings.get("leave_patterns", []) or []
    leave_res: list[re.Pattern[str]] = []
    for pat in _leave_src:
        try:
            leave_res.append(re.compile(pat))
        except re.error:
            # Bad regex in settings — skip rather than crash the whole metric
            continue

    def _classify(label: str | None) -> str:
        """Four-way classification for unavail blocks:
          * 'leave'        → subtracts from util denom AND from days_worked
                              (AL/SL/PL/PH and common public holidays)
        ...
        """
        text = label or ""
        # Leave FIRST — if a block is tagged with a leave keyword, that wins
        # over any other classification (e.g. "AL (billable catch-up)"
        # should be leave, not admin).
        if any(r.search(text) for r in leave_res):
            return "leave"
        low = text.lower()
        if any(k in low for k in qualify):
            return "qualifying"
        if any(k in low for k in breaks):
            return "break"
    ...
    for (pid, day), day_grp in delivered.groupby(["practitioner_id", "local_date"]):
        ...
        for b in unavail_blocks:
            ...
            if b["kind"] == "qualifying":
                ...
                # v26.7 — leave (AL/SL/PL/PH etc) subtracts from util denom
                # (like any non-qualifying block) AND from days_worked
                # (like breaks). Kept in its own tally so white_space
                # computation can subtract it cleanly.
                nonqual_day_min += mins
                leave_day_min += mins
            else:
                nonqual_day_min += mins
...
def white_space_hours_map(cells: pd.DataFrame) -> dict[str, float]:
    ...
    v26.7 — two categories subtract from days_worked:
      ...
      * Leave unavail (AL/SL/PL/PH and common Aussie public holidays —
        controlled by ``leave_patterns`` in settings.yml)
        → not available, SUBTRACTED.

    So ``white_space_hours = window − break_min − leave_min``. Utilisation
    denom is computed independently and counts both break and leave as
    non-qualifying (so neither drags the % down).
    """
    if cells is None or cells.empty:
        return {}
    g = cells.copy()
    # Defensive — older cell tables (pre-v26.6.2) may be missing these cols
    # when loaded from a stale Streamlit cache.
    if "break_min" not in g.columns:
        g["break_min"] = 0.0
    if "leave_min" not in g.columns:
        g["leave_min"] = 0.0
    g["_white_min"] = (g["window_min"]
                       - g["break_min"].fillna(0.0)
                       - g["leave_min"].fillna(0.0))
