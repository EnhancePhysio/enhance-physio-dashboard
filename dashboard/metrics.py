...
def _fetch_treatment_notes_for_range(
    ...
    try:
        for n in client.paginate("treatment_notes", params=params):
            appt_id = (
                extract_linked_id(n.get("appointment"), "self")
        ...
        return pd.DataFrame(columns=[
            "appointment_id", "note_created_at", "note_archived_at",
        ])
    ...
    # If an appointment has multiple notes (addendum after finalisation),
    # keep the EARLIEST created_at. That's the true first-finalisation time
    # and the one that matters for the 24h medicolegal window.
    df = (
        df.sort_values("note_created_at")
...
def notes_completion(client: ClinikoClient, appts: pd.DataFrame, dr: DateRange) -> pd.DataFrame:
    ...
    if used_note_endpoint:
        # Left-join: one row per delivered appt, optional note_created_at
        merged = delivered.merge(
            notes_df, left_on="id", right_on="appointment_id", how="left",
        )
        delta_h = (
            (merged["note_created_at"] - merged["starts_at"]).dt.total_seconds()
            / 3600.0
        )
        ...
        # AND that note was created within the window.
        has_note = merged["note_created_at"].notna()
        merged["_passed_24h"] = (
            merged["_expected"] & has_note & within_window
        )
        base = merged
    else:
