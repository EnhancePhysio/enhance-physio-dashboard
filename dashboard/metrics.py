...
def _compute_white_windows(client: ClinikoClient, appts: pd.DataFrame,
    ...
    def _classify(label: str | None) -> str:
        ...
        # Anything else (lunch, break, generic hold) is still non-qualifying.
        return "non_qualifying"
...
def white_space_hours_map(cells: pd.DataFrame) -> dict[str, float]:
    """Practitioner → bookable-clinical-hours total across the range.

    "White space" = window − qualifying − non-qualifying unavail
                  = purely the time that's available to see patients.
    Drives the v26.6 days_worked calculation (hours ÷ 6.6).
    """
    if cells is None or cells.empty:
        return {}
    g = cells.copy()
...
def merge_per_practitioner(result: MetricResult,
    ...
    for df, cols in [
        ...
        (result.white_space_hours, ["practitioner_id", "white_space_hours"]),
