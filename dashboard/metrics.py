...
def new_patients(client: ClinikoClient, appts: pd.DataFrame, dr: DateRange,
    ...
    # Cliniko ignores `q[]=created_at:...` on /patients but DOES honour
    # `q[]=updated_at:...`. Every new patient has updated_at >= created_at,
    # so filtering by updated_at in the range catches all newly-created
    # patients (plus some older ones that were touched — we filter client-
    # side by created_at to finalise).
...
# -------------------------------------------------------------------
def _note_is_finalised(n: dict[str, Any]) -> bool:
    """Treat a treatment_note payload as finalised.

    Cliniko has shipped at least three shapes for this:
      • status: "Finalised" / "Draft"
      • status: 90 / 20 (numeric — matches appointment status codes)
      • draft: true/false (legacy)
    We handle all three and default to True when nothing is present
    (conservative — a missing flag on a real note has almost always meant
    finalised in practice, and we'd rather pass a real finalised note than
    silently drop it).
    """
    st = n.get("status")
    if isinstance(st, str):
        return st.strip().lower() in {"finalised", "finalized", "final", "complete", "completed"}
    if isinstance(st, (int, float)):
        return int(st) == 90
    if "draft" in n:
        return not bool(n["draft"])
    return True


def _finalised_stamp(n: dict[str, Any]) -> str | None:
    """Return the best-guess ISO string for when a note was finalised.

    Cliniko exposes `finalised_at` on some API versions but not others.
    Fall through: finalised_at → updated_at → created_at.
    """
    for k in ("finalised_at", "finalized_at", "updated_at", "created_at"):
        v = n.get(k)
        if v:
            return v
    return None
...
def notes_completion(client: ClinikoClient, appts: pd.DataFrame, dr: DateRange) -> pd.DataFrame:
    ...
    Denominator: delivered appointments where `treatment_note_status != 10`
    (10 = N/A, no note expected). Using the status field is far more
    reliable than guessing from appointment type.

    Numerator: of those expected appts, the count whose treatment note was
    finalised within `window_hours` (default 24) of the appointment starting.
    We get the finalisation timestamp by fetching treatment notes per patient
    (the only Cliniko endpoint that reliably returns notes) and matching
    each note back to its appointment via the embedded `appointment` link.

    Fallback: if the /treatment_notes endpoint returns nothing — an auth or
    rate-limit issue — we use the appointment's own `updated_at` as a proxy
    (Cliniko bumps updated_at when the status transitions 20/30 → 90). This
    is less precise but prevents the metric from flat-lining at 0%.
    """
    if appts.empty:
    ...
    # --- Denominator: appts where a note was expected (status != 10 = N/A)
    status = pd.to_numeric(delivered["treatment_note_status"], errors="coerce")
    # Treat missing/NaN status as "expected" — conservative, matches Cliniko
    # behaviour where the field is populated for all real consults.
    delivered["_expected"] = status.fillna(20).astype(int) != 10
    delivered["_is_finalised_eventually"] = status.fillna(0).astype(int) == 90

    # --- Gather treatment notes per patient and index by appointment_id
    notes_by_appt: dict[str, list[dict]] = {}
    patient_ids = [pid for pid in delivered["patient_id"].dropna().unique() if pid]
    ...
    # --- Per-appt compliance check
    def _within_24h_via_note(row) -> bool | None:
        """Return True/False if we can judge via a matched note, else None."""
        aid = row.get("id")
        if aid is None:
            return None
        notes = notes_by_appt.get(str(aid), [])
        if not notes:
            return None
        starts_at = row.get("starts_at")
        if pd.isna(starts_at):
            return None
        appt_dt = starts_at.to_pydatetime()
        for n in notes:
            if not _note_is_finalised(n):
                continue
            stamp = _finalised_stamp(n)
            if not stamp:
                continue
            try:
                ndt = datetime.fromisoformat(stamp.replace("Z", "+00:00"))
            except ValueError:
                continue
            delta_h = (ndt - appt_dt).total_seconds() / 3600.0
            # Allow a small negative buffer in case the note was pre-written
            # or the appt start time was amended post-hoc (rare but happens).
            if -1.0 <= delta_h <= window_h:
                return True
        return False

    def _within_24h_via_appt(row) -> bool:
        """Fallback using the appt's own updated_at timestamp."""
        if int(status.loc[row.name] if row.name in status.index else 0) != 90:
            return False
        au = row.get("appt_updated_at")
        sa = row.get("starts_at")
        if pd.isna(au) or pd.isna(sa):
            return False
        delta_h = (au - sa).total_seconds() / 3600.0
        return -1.0 <= delta_h <= window_h

    # Build the per-row pass/fail result, preferring note-based evidence.
    passed: list[bool] = []
    for _, row in delivered.iterrows():
        if not row["_expected"]:
            passed.append(False)  # excluded from numerator anyway
            continue
        via_note = _within_24h_via_note(row)
        if via_note is not None:
            passed.append(via_note)
        else:
            # No note matched — fall back to the appt-level signal.
            passed.append(_within_24h_via_appt(row))
    delivered["_passed_24h"] = passed
